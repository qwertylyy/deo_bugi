...
{block name='page_title_sidebar'}
	<h1 class="page-title">{l s='Order details' d='Shop.Theme.Customeraccount'}</h1>
{/block}
...
{block name='page_right_content'}
	{block name='order_infos'}
		<div id="order-infos">
			<div class="box">
				<div class="row">
					<div class="col-xs-{if $order.details.reorder_url}9{else}12{/if} col-sp-12">
						<strong class="reorder-reference">
							{l
					...
					{if $order.details.reorder_url}
						<div class="col-xs-3 col-sp-12 text-sp-right">
							<a href="{$order.details.reorder_url}" class="btn btn-outline">{l s='Reorder' d='Shop.Theme.Actions'}</a>
						</div>
					{/if}
				</div>
			</div>
			...
			<div class="box box-border">
				<ul>
					<li><label>{l s='Carrier' d='Shop.Theme.Checkout'}</label> {$order.carrier.name}</li>
					<li><label>{l s='Payment method' d='Shop.Theme.Checkout'}</label> {$order.details.payment}</li>
	...
	{block name='order_history'}
		<section id="order-history" class="box">
			<h3 class="text-sp-center">{l s='Follow your order\'s status step-by-step' d='Shop.Theme.Customeraccount'}</h3>
	...
	{block name='order_carriers'}
		{if $order.shipping}
			...
			<div class="hidden-md-up shipping-lines box box-border">
				{foreach from=$order.shipping item=line}
