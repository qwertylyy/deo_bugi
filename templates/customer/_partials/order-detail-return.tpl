...
{block name='order_products_table'}
  <form id="order-return-form" class="js-order-return-form" action="{$urls.pages.order_follow}" method="post">
      ...
      <table id="order-products" class="table table-bordered return">
        ...
        {foreach from=$order.products item=product name=products}
          <tr>
            ...
            <td class="text-sp-right">{$product.qty_returned}</td>
            <td class="text-sp-right">{$product.price}</td>
            <td class="text-sp-right">{$product.total}</td>
          </tr>
        {/foreach}
        <tfoot>
          {foreach $order.subtotals as $line}
            {if $line.value}
              <tr class="text-sp-right line-{$line.type}">
                <td colspan="5">{$line.label}</td>
                <td colspan="2">{$line.value}</td>
              </tr>
            {/if}
          {/foreach}
          <tr class="text-sp-right line-{$order.totals.total.type}">
    ...
    <div class="box">
      <div class="order-items hidden-md-up">
              ...
              <div class="content">
                <div class="row">
                  ...
                  <div class="col-sm-7 qty">
                    <div class="row">
                      <div class="col-sp-4 text-sm-left text-sp-left">
                        {$product.price}
                      </div>
                      <div class="col-sp-4">
                        {if $product.customizations}
                      ...
                      <div class="col-sp-4 text-sp-right">
                        {$product.total}
                      </div>
                    </div>
                  </div>
      ...
      <div class="order-totals hidden-md-up">
        {foreach $order.subtotals as $line}
          {if $line.value}
            <div class="order-total row">
              <div class="col-sp-8"><strong>{$line.label}</strong></div>
              <div class="col-sp-4 text-sp-right">{$line.value}</div>
            </div>
          {/if}
        {/foreach}
        <div class="order-total row">
          <div class="col-sp-8"><strong>{$order.totals.total.label}</strong></div>
          <div class="col-sp-4 text-sp-right">{$order.totals.total.value}</div>
        </div>
      </div>
    </div>
    ...
    <div class="box">
      ...
      <footer class="form-footer">
        ...
        <button class="form-control-submit btn btn-outline" type="submit" name="submitReturnMerchandise">
