...
{else}
  {assign var=arr_input value=["select", "countrySelect", "radio-buttons", "checkbox", "date", "birthday"]}
  {if isset($icon) && $icon && !$field.type|in_array:$arr_input}
    {assign var=hasicon value=true}
  {/if}

  <div class="form-group row{if isset($hasicon) && $hasicon} form-group-input {$field.name}{/if}{if !empty($field.errors)} has-error{/if}">
    <label class="col-md-12 form-control-label{if $field.required} required{/if}" for="field-{$field.name}">
      {if $field.type !== 'checkbox'}
        {$field.label}
      {/if}
    </label>
   
    <div class="col-md-12 form-input{if ($field.type === 'radio-buttons')} form-control-valign{/if}{if isset($hasicon) && $hasicon} input-group{/if}">
      {if isset($hasicon) && $hasicon}
        <div class="input-group-addon">
          <i class="icon icon-{$field.name}"></i>
        </div>
      {/if}
    ...
    {block name='form_field_errors'}
      <div class="col-md-12 form-control-errors">
