  ...
  {foreach from=$groups key=id_attribute_group item=group}
    ...
    <div class="clearfix product-variants-item">
        ...
        <ul id="group_{$id_attribute_group}">
          {foreach from=$group.attributes key=id_attribute item=group_attribute}
            <li class="float-sp-left input-container">
              <label aria-label="{$group_attribute.name}" data-toggle="ui-tooltip" data-position="top" title="{$group.name}: {$group_attribute.name}">
      ...
      {elseif $group.group_type == 'radio'}
        <ul id="group_{$id_attribute_group}">
          {foreach from=$group.attributes key=id_attribute item=group_attribute}
            <li class="input-container float-sp-left">
              <label data-toggle="ui-tooltip" data-position="top" title="{$group.name}: {$group_attribute.name}">
