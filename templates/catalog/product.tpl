...
{/block}
{if isset($product.layout_fullwidth) && $product.layout_fullwidth}
  {block name='deo_fullwidth_hook_open'}{/block}
  {block name='deo_fullwidth_hook_close'}{/block}
{/if}

{block name='content'}
  {if isset($product.productLayout) && $product.productLayout != ''}
    {hook h='displayDeoProfileProduct' product=$product typeProduct='detail'}
  {else}
                  ...
                  <div class="col-lg-3 col-md-4 col-sm-6 col-xs-6 col-sp-12 ajax_block_product">
                    {block name='product_miniature'}
                      {if isset($productProfileDefault) && $productProfileDefault}
                          ...
                          {hook h='displayDeoProfileProduct' product=$product_accessory profile=$productProfileDefault  key="position"}
                      {else}
                          {include file='catalog/_partials/miniatures/product.tpl' product=$product_accessory  key="position"}
                      {/if}
                    {/block}
                  </div>
                {/foreach}
