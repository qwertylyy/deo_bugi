  ...
  <div class="delivery-options-list">
    {if $delivery_options|count}
      ...
      >
            ...
            <div class="delivery-options">
              {foreach from=$delivery_options item=carrier key=carrier_id}
                  <div class="row delivery-option js-delivery-option">
                    <div class="col-xs-2 col-sm-1 col-md-2 text-sp-center">
                      <span class="custom-radio float-sp-left">
                        <input type="radio" name="delivery_option[{$id_address}]" id="delivery_option_{$carrier.id}" value="{$carrier_id}"{if $delivery_option == $carrier_id} checked{/if}>
                        <span></span>
                      </span>
                    </div>
                    <label for="delivery_option_{$carrier.id}" class="col-xs-10 col-sm-11 col-md-10 delivery-option-2 label-inherit">
                      <div class="row">
                        <div class="col-sm-5 col-xs-12">
                          <div class="row carrier{if $carrier.logo} carrier-hasLogo{/if}">
                            {if $carrier.logo}
                            <div class="col-xs-12 col-md-4 carrier-logo">
                                <img src="{$carrier.logo}" alt="{$carrier.name}" loading="lazy" class="img-fluid"/>
                            </div>
                            {/if}
          ...
          <div class="order-options">
            <div id="delivery">
              <div class="form-group">
                <label for="delivery_message" class="label-inherit">{l s='If you would like to add a comment about your order, please write it in the field below.' d='Shop.Theme.Checkout'}</label>
                <textarea rows="2" cols="120" class="form-control"  id="delivery_message" name="delivery_message">{$delivery_message}</textarea>
              </div>
            </div>
        ...
        <button type="submit" class="continue btn btn-outline float-xs-right" name="confirmDeliveryOption" value="1">
